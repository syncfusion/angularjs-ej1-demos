...
syncApp.controller('PivotGridFrozenCtrl', function ($scope) {
    $scope.dataSource = {
        ...
        columns: [{ fieldName: "[Date].[Fiscal]" }, { fieldName: "[Geography].[Geography]" }],
    ...
    $scope.renderGrid = function (args) {
        ...
        if (rdbRelational.model.checked) {
            ...
            pivotGrid.element.css({ "height": "350px", "width": "100%", "float": "left" });
            pivotGrid.model.frozenHeaderSettings = { enableFrozenColumnHeaders: chkFrozenCol.checked(), enableFrozenRowHeaders: chkFrozenRow.checked(), enableFrozenHeaders: (chkFrozenCol.checked() && chkFrozenRow.checked()) ? true : false };
        }
        else {
            pivotGrid.model.dataSource = {
                data: "//bi.syncfusion.com/olap/msmdpump.dll", catalog: "Adventure Works DW 2008 SE", cube: "Adventure Works",
