...
isPopupOpened = false;
var culture_switch = [{ id: "1", locale: "en-US" }, { id: "2", locale: "de-DE" }, { id: "3", locale: "es-ES" }, { id: "4", locale: "fr-FR" }];
...
initControls = function () {
    ...
    $.ajax({
        url: 'scripts/l10n/ej.localetexts.' + $("#culture_switcher").val() + '.js',
        async: false
    });
...
$(function () {
    ...
    var preLoadSample = function (control, currentSample, categories) {
    ...
    var loadSamplePage = function (control, currentSample, categories) {
        ...
        if (ej.isNullOrUndefined(control)) {
            ...
            bindUnbindDocClickEvents(true);
            document.title = "Essential JS 1 for AngularJS";
            return;
        }
        if (isloadLeft || Number(name.id) <= 1)
            self.loadLeftTab(control + "__sb", currentSample);
        preLoadSample(control, currentSample, categories);
    };
...
function viewdemo(product) {
    ...
    else
    {
        if (location.protocol == "file:") {
            ...
            else window.location.href = "../../ionic1%20app/desktop/www/index.html";
        }
        else if (location.toString().indexOf("sfjavascriptsamplebrowser") != -1) {
            if (product == "desktop") return;
            else window.location.href = "//localhost:" + window.location.port + "/sfionicjsdesktop"
        }
        else {
            var links = $(document.head || document.getElementsByTagName('head')[0]).find("link");
            ...
            var path = "JavaScript\\ionic1 app\\desktop\\www";
            $.ajax({
                async: false,
                cache: true,
        ...
        function copycontent(e) {
            if ($("#copytextarea").val() == "" || $("#copytextarea").val() == null) {
                ...
                $("#sourceTab >div:visible").each(function () {
                    $("#copytextarea").val($("#copytextarea").val() + $(this).text().replace("Copy", ""));
                });
            }
