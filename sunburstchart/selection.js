...
syncApp.controller('SunburstCtrl', function ($scope) {
    $scope.ndatasource = selection_DataSource; 
	$scope.levels = [
						{groupMemberPath:"Continent"},
						{groupMemberPath:"Country"},
						{groupMemberPath:"State"}
	];	
	$('#highlightType').change(function () {
        var type = $("#highlightType option:selected").text();
	...
	$('#selectionType').change(function () {
        var type = $("#selectionType option:selected").text();
        var sunburst = $("#container").ejSunburstChart("instance");
        switch (type) {
            case 'Point':
