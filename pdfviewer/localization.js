syncApp.controller('PDFViewerLocalizationController', function ($scope, $interval) {
    $scope.serviceurl = window.baseurl + "api/PdfViewer";
    $scope.pdfService = ej.PdfViewer.PdfService.Local;
    $scope.dropDownChange = function (sender) {
        var _ejPdfViewer = $("#container").data("ejPdfViewer");
        _ejPdfViewer.setModel({ locale: sender.value });
    };
});
