/* Control Rendering */
angular.module('hospitalMgnt', ['ejangular']).controller('HospitalCtrl', function ($scope) {
    ...
    $scope.setDate = new Date(2016, 3, 1);
    $scope.startSearchDate = new Date(2016, 3, 1);
    $scope.endSearchDate = new Date(2016, 3, 1);
    $scope.appwincategory = window.categorylist;
    $scope.departdata = window.departmentName;
    $scope.windowdatafields = { text: "Text", id: "Id", value: "Text" };
    $scope.recurfrequencies = ["daily", "weekly", "monthly", "yearly", "everyweekday"];
    $scope.categorydata = categoryclone;
    $scope.datafields = { text: "Text" };
    $scope.categorytemplate = "<div id='clist' class='categoryname'>${Text}</div><div style='float:right;'><i class='icon-category' style='color:${Color}'></i></div>";
    $scope.doctordesidata = doctorclone;
    $scope.doctordesifields = { text: "Designation" };
    $scope.doctordata = window.doctorName;
    $scope.doctortemplate = "<div id='doctors'><div id='dtrimage'><img class='doctorimg' src='../../content/images/hospitalmanagement/${Text}.png' alt='doctor'/></div><div id='dtrdetails'><div class='doctorname'>${Name}</div><div class='designation'>${Designation}</div></div><div id='dtrcategory'><i class='icon-${Value}'></i></div></div>";
    $scope.searchdata = ["ID", "Name", "Description", "Category", "Date"];
    $scope.waitdata = window.waitinglist;
    $scope.waitfields = { text: "Subject" };
    $scope.waitingtemplate = "<div id='waiting'><div id='waiticon'><div class='icon-waiticon'></div></div><div id='waitid'>${Id}</div><div id='waitdetails'><div id='waitlist'>${Subject}</div><div id='waitcategory'>${dept} - ${Category}</div></div><div id='waitcategoryicon'><i class='icon-category' style='color:${Categorycolor}'></i></div></div>";
    $scope.appointments = window.hospitaldata;
    $scope.schViews = ["Day", "Week", "Month"];
    ...
    $scope.res2 = {
        dataSource: $.extend(true, [], window.doctorName),
        text: "Text", id: "Id", groupId: "GroupId", color: "Color"
    };
});
...
$(document).ready(function () {
    ...
    ej.Schedule.prototype._renderAppointmentCategory = function () {
        return;
    }
});
...
function _appcolor(cat) {
    return new ej.DataManager(window.categorylist).executeLocal(new ej.Query().where("Value", ej.FilterOperators.equal, cat))[0].Color;
}
...
function onListCheck(args) {
    ...
    if (args.item.parent().attr("id") == "categorylist" && categoryCheck) {
        ...
        var newdoctorlist = $.extend(true, [], this.dataSource());
        var newlist = [];
    ...
    var doctorlistChecked = [];
    for (var a = 0; a < docObj.dataSource().length; a++) {
        for (var b = 0; b < window.doctorName.length; b++) {
            if (window.doctorName[b].Id == docObj.dataSource()[a].Id) {
                var index = b;
                break;
            }
        }
